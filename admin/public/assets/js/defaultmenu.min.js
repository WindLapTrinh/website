...
function setNavActive() {
  // Lấy phần đường dẫn sau 'public/' trong URL
  let currentPath = window.location.pathname.split("/public/")[1];

  console.log('Current Path:',currentPath );
  if (!currentPath) { // Nếu không có phần đường dẫn sau 'public/', giả sử đây là trang chủ
    currentPath = "index"; // Bạn có thể điều chỉnh lại theo tên trang chủ của mình
  }

  let sidemenuItems = document.querySelectorAll(".side-menu__item");
  
  sidemenuItems.forEach((e) => {
    // Chuẩn hóa so với href trong menu
    let menuPath = e.getAttribute("href").replace("/", "").replace(".html", "");
    ...
    if (currentPath === menuPath) {
      ...
      // Duyệt qua các phần tử cha để set active
      while (hasParent) {
        if (parent) {
          ...
          if (parent.parentElement.classList.contains("has-sub")) {
            let elemRef = parent.parentElement.parentElement.parentElement;
            elemRef.classList.add("open", "active");
            elemRef.firstElementChild.classList.add("active");
            elemRef.children[1].style.display = "block";

            Array.from(elemRef.children[1].children).forEach((element) => {
          ...
          if (parent.classList.contains("child1")) {
            slideDown(parent); // Nếu có animation thì slide xuống
          }
