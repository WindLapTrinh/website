...
class AdminUserController extends Controller
{
    ...
    function list(Request $request)
    {
        ...
        // dd($users->total());
        return view('admin.user.list', compact('users', 'count', 'request', 'list_act', 'roles'));
    }
    ...
    function store(Request $request)
    {
        $request->validate(
            [
                ...
                'role_id' => ['required', 'array', 'min:1'], // Kiểm tra xem role_id là mảng và không rỗng
            ],
            [
        ...
        // Kiểm tra nếu role_id tồn tại và không rỗng trước khi đồng bộ
        if ($request->has('role_id') && !empty($request->input('role_id'))) {
            $user->roles()->sync($request->input('role_id'));
        }

        return redirect()->route('user.list')->with('satus','Bạn đã thêm nhân sự thành công');
    }
