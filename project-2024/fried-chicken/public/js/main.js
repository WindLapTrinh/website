...
document.addEventListener('DOMContentLoaded', () => {
    const savedValue = localStorage.getItem('selectedPrice');
    
    if (savedValue) {
        const selectedRadio = document.querySelector(`input[name="price"][value="${savedValue}"]`);
        if (selectedRadio) {
            selectedRadio.checked = true;
        }
    }
    
    const radios = document.querySelectorAll('#filterProductForm input[type="radio"]');
    
    radios.forEach(input => {
        input.addEventListener('change', function() {
            localStorage.setItem('selectedPrice', this.value);
            // Submit the form immediately when the radio button is changed
