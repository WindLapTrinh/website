                        ...
                        <ul class="list-item">
                            @if ($posts->count() > 0)
                                @foreach ($posts as $post)
                                    <li class="clearfix">
                                        <a href="{{ route('post.detail', ['post' => $post->slug]) }}" title="" class="thumb fl-left">
                                            <img src="{{ asset($post->image->url) }}" alt="">
                                        </a>
                                        <div class="info fl-right">
                                            <a href="{{ route('post.detail', ['post' => $post->slug]) }}" title="" class="title">{{ $post->title }}</a>
                                            <span class="create-date">{{ $post->created_at->format('d/m/Y') }}</span>
                                            <p class="desc">{{ $post->note }}</p>
                                        </div>
                                    </li>
                                @endforeach
                            @else
